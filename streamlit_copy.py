...
st.set_page_config(page_title="우리동네 가성비", page_icon=":house:", layout="wide")
...
def initialize_components():
    ...
    2. 지역과 가게 이름 구분:
        ...
        - 예를 들어, 사용자가 '인천'을 물어본다면, '인천광역시'를 포함하는 주소를 가진 가게를 추천하십시오.
        - 예를 들어, 사용자가 '인천시'를 물어본다면, '인천광역시'를 포함하는 주소를 가진 가게를 추천하십시오.
        - 예를 들어, 사용자가 '**'를 물어본다면, '**광역시'를 포함하는 주소를 가진 가게를 추천하십시오.
        - 예를 들어, 사용자가 '서울'를 물어본다면, '서울특별시'를 포함하는 주소를 가진 가게를 추천하십시오.
        - 예를 들어, 사용자가 '서울시'를 물어본다면, '서울특별시'를 포함하는 주소를 가진 가게를 추천하십시오.
...
def main():
    #st.set_page_config(page_title="우리동네 가성비", page_icon=":house:", layout="wide")
    st.title("우리동네 가성비")
    st.markdown("### **가성비 좋은 가게를 추천해드릴게요!**")
    st.markdown("질문을 입력하고 **'질문하기'** 버튼을 눌러주세요. :speech_balloon:")

    # 사이드바 설정 (부가 정보나 도움말 제공 가능)
    with st.sidebar:
        st.markdown("## :mag: **도움말**")
        st.markdown("""
        - 가게 이름, 지역, 주소 등을 입력하여 질문하세요.
        - 예: "**에서 10000원 이하 메뉴가 있는 가게 있어?", "대전시에서 추천해줄 가게 있어?"
        - 질문하기 버튼을 눌러 대화를 시작하세요!
        """)
        st.markdown("### :phone: **문의사항** 010-\\*\\*\\*\\*-\\*\\*\\*\\*")
        st.markdown("궁금한 점이 있으면 언제든 연락주세요!")

    # 메인 질문 입력 및 버튼
    question = st.text_input("질문 :", "", placeholder="여기에 질문을 입력하세요.")
    if st.button("질문하기:speech_balloon:", type="primary"):
        if question:
    ...
    if 'chat_history' in st.session_state:
        ...
        for i, chat in enumerate(st.session_state.chat_history):
            # 사용자 메시지
            message(f"{chat['user']}", is_user=True, key=f"user_{i}")
            # 챗봇 메시지
            message(f"{chat['bot']}", key=f"bot_{i}")
        st.markdown("---")
